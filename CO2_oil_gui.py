...
class PredictionEngine:
    ...
    def predict(self, values: dict) -> tuple:
        X = self._row(values)
        co2 = self._predict_one(self.co2_model, self.co2_kind, X, values, which="co2")
        oil = self._predict_one(self.oil_model, self.oil_kind, X, values, which="oil")
        return round(float(co2), 6), round(float(oil), 6)
    ...
    def _load_matlab_json(self, path: str):
        ...
        iw = np.asarray(nn["IW"], dtype=float)                 # hidden x features
        lw = np.asarray(nn["LW"], dtype=float).reshape(1, -1)  # 1 x hidden
        b = nn["b"]
        b1 = np.asarray(b[0], dtype=float).reshape(-1)         # hidden
        b2 = float(np.asarray(b[1], dtype=float))              # scalar
    ...
    def _predict_one(self, model, kind: str, X: np.ndarray, values: dict, which: str):
        if model is None:
            ...
            return 5.0 * values["Perm_mD"] * (1 - values["S_org"]) * (1 - values["S_owv"]) + 0.1 * values["BHP_psi"]
        if kind == "json":
...
class InputTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.sowv    = self._add_numeric(grid, row, "S_owv",               0.0, 1.0, 0.01, 4, 0.0);      row += 1
...
class OutputTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.lbl_co2 = QLabel("CO₂ Storage (ton):")
        self.val_co2 = QLabel("—")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("CO2 and Oil Predictor")
...
def main():
    ...
    app.setApplicationName("CO2-Oil Predictor")
    window = MainWindow()
    window.show()
    sys.exit(app.exec_())
